...
* @param elfHdr
*/
static void print_s_flag(Elf64_Ehdr *elfHdr)
{
	int fanion = 0;
	char *type = NULL;
	printf ("format de fichier elf64-x86-64\n");
...
* @param SectNames
*/
static bool print_data(Elf64_Shdr *sectHdr, FILE *ElfFile, Elf64_Ehdr *elfHdr,
			char *SectNames)
{
	uint32_t idx;
	print_s_flag(elfHdr);
	...
	for (idx = 0; idx < elfHdr->e_shnum; idx++) {
		fseek(ElfFile, elfHdr->e_shoff + idx * sizeof(Elf64_Shdr),
			SEEK_SET);
		if (fread(sectHdr, 1, sizeof(Elf64_Shdr), ElfFile)
			< sizeof(Elf64_Shdr))
			return false;
		if ((sectHdr->sh_type == SHT_PROGBITS &&
			strcmp(".note.GNU-stack", SectNames + sectHdr->sh_name)
			!= 0) || elfHdr->e_type == ET_EXEC) {
			if (!print_sect64(sectHdr, SectNames, ElfFile))
				return false;
		}
	}
	return true;
...
*/
bool get_objdump64(FILE *ElfFile, Elf64_Ehdr *elfHdr, struct stat *s, char *fil)
{
	char *SectNames;
	Elf64_Shdr sectHdr;
	size_t check;
